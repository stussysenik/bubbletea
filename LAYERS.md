...
| View Tree and Layout | Done | zig/src/ui.zig | The shared scene graph can render rows, columns, boxes, spacers, rules, and semantic tones across terminal, headless, and browser hosts, exposes measured layout bounds inside structured snapshots, and can tag interactive regions and item actions for browser hosts. | Push the same metadata deeper into field-level interactions and richer layout metadata so hit testing, scrolling regions, and cross-host renderers can share more than terminal-sized boxes. |
| Renderer | In Progress | zig/src/renderer.zig<br>zig/src/ui.zig | ANSI rendering now parses the composed frame into styled cells, diffs per-glyph runs instead of whole lines, and still enables the terminal modes needed for richer input. | Add wide-cell/grapheme awareness, cursor-state rendering, and host-specific effects on top of the new cell grid. |

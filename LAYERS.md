...
| Renderer | In Progress | zig/src/renderer.zig<br>zig/src/ui.zig<br>zig/src/cell_width.zig | ANSI rendering now parses the composed frame into styled cells, diffs per-glyph runs instead of whole lines, keeps wide glyphs and combining sequences aligned to real terminal columns, and still enables the terminal modes needed for richer input. | Add true grapheme-cluster edge cases, cursor-state rendering that can drive the real terminal cursor, and host-specific effects on top of the new cell grid. |
...
- Add true grapheme-cluster edge cases and real terminal-cursor state on top of the new wide-glyph-safe cell-buffer renderer.
